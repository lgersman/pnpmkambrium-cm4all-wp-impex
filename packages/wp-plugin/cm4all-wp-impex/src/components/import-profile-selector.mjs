...
export default function ImportProfileSelector({ value, onChange }) {
  ...
  const setImportProfile = (importProfileName = null) => {
    const importProfile = importProfiles.find((_) => _.name === importProfileName);
  ...
  if (importProfiles.length > 1) {
    options.unshift({
      name: importProfiles.length ? __('Select an import profile', 'cm4all-wp-impex') : __('No import profiles found'),
      disabled: true,
    });
  }
  ...
  return (
    <wp.components.SelectControl
      ...
      help={__('Import profiles define which WordPress data should be consumed', 'cm4all-wp-impex')}
    ></wp.components.SelectControl>
  );
}
