...
ScreenContext = {
  ...
  async saveSlicesChunk(exportDirHandle, response, chunk) {
    ...
    const chunkDirHandle = await exportDirHandle.getDirectoryHandle(`chunk-${chunk.toString().padStart(4, '0')}`, {
      create: true,
    });

    return Promise.all(
      slices.map(async (slice, index) => {
        const sliceFileHandle = await chunkDirHandle.getFileHandle(`slice-${index.toString().padStart(4, '0')}.json`, {
          create: true,
        });
...
export function ScreenContextProvider({ children }) {
  return <ContextProvider.Provider value={ScreenContext}>{children}</ContextProvider.Provider>;
}
