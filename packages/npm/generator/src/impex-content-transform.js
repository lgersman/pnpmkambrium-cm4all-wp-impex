  ...
  setup(configuration = {}) {
    ...
    if (onRegisterCoreBlocks() !== false) {
      ...
      originalBlockTransforms.forEach((originalBlockTransform) => {
        coreBlocks.find((coreBlock) => coreBlock?.name === originalBlockTransform?.name).settings.transforms =
          cloneDeepWith(originalBlockTransform.settings.transforms, (value, indexOrKey, stack) => {
