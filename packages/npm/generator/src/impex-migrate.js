...
async function processSlice(sliceCallback, slicePath, pathGenerator, targetPath, options) {
  ...
  if (!(await sliceCallback(slicePath, pathGenerator, targetPath, options))) {
    ...
    entries = entries.filter((entry) => entry.name.startsWith(sliceFilenameBase));
    ...
    for (const entry of entries) {
      await cp(join(sourceSliceDir, entry.name), targetSlicePath + entry.name.substring(sliceFilenameBase.length), {
        force: true,
        recursive: true,
      });
    }
...
export default async function migrate(sourcePath, targetPath, sliceCallback, options = {}) {
  ...
  const _processSlice = async (slicePath) =>
    await processSlice(sliceCallback, slicePath, pathGenerator, targetPath, options);
