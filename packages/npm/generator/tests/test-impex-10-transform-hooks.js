...
import Transformer, { traverseBlocks } from '../src/impex-content-transform.js';

import { addFilter } from '@wordpress/hooks';
import { getBlockAttributes, createBlock } from '@wordpress/blocks';

const VERBOSE = false;

test("ImpexTransformer: test 'core/image' content transform", (t) => {
  ...
  t.ok(transformed.includes(' title="Fly fishing"/>'));
  t.match(transformed, /<!-- \/wp:image -->$/);

  t.end();
});

test('ImpexTransformer: onRegisterCoreBlocks hook : takeover img[@title] as figcaption', (t) => {
  Transformer.setup({
    verbose: VERBOSE,
  ...
  t.ok(transformed.includes('<figcaption>Fly fishing</figcaption>'));
...
test("ImpexTransformer: onSerialize hook : 'core/image' transform", (t) => {
  Transformer.setup({
    verbose: VERBOSE,
    onSerialize(blocks) {
...
test('ImpexTransformer: onSerialize hook : surround <img> with <figure>', (t) => {
  Transformer.setup({
    verbose: VERBOSE,
    onDomReady(document) {
  ...
  t.notOk(transformed.includes('title="Fly fishing"'));
  t.match(transformed, /^<!-- wp:image -->/);
  t.ok(transformed.includes('<figcaption>Fly fishing</figcaption>'));
  t.match(transformed, /<!-- \/wp:image -->$/);
...
test('ImpexTransformer: onLoad hook : surround <img> with <figure>', (t) => {
  Transformer.setup({
    verbose: VERBOSE,
    onLoad(html) {
      // replace <img> with <figure> by dumb regex replace operations
      return html.replace(/<img([^>]*)\/>/g, (img, attributes) => {
        const attributesMap = [...attributes.matchAll(/\s*(?<name>[^=]+)="(?<value>[^"]+)"/g)]
