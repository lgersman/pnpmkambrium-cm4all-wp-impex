...
test('migrate() : test noop migration', async (t) => {
  ...
  t.deepEqual(
    slicePaths,
    new Array(slicePaths.length).fill('').map((_, index) => `slice-${('' + index).padStart(4, '0')}.json`),
  );
  ...
  try {
    ...
    t.fail(
      `migrated impex export directory content is not identical : "diff -r ${__SNAPSHOT_HECHT} ${__TMP_DIR}/noop-migration" (exitCode=${ex.exitCode},stderr="${ex.stderr}",stdout="${ex.stdout}")`,
    );
  }
